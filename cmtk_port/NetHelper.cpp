...
int	GetNetip(device_t *list)
{
	...
	for ( int i = 0; i < n; ++i, ++pifr ) 
	{
		...
		memcpy (list + nRet, &_device, sizeof(device_t));
		nRet++;
	}
out:
	free( _ifc.ifc_buf);
...
#ifdef _WIN32
int	GetNetip(/*unsigned int *ip*/device_t *list)
{
	...
	{ 
		...
		{ 
			...
			list[i].ip = *(unsigned long*)host->h_addr_list[i];	
